...
$query = "UPDATE `goods` SET `name` = $name, `short_description` = $short_description,
`ingridients` = $ingridients, `specification`= $specification, `gost` = $gost WHERE `barcode` = $barcode";
...
<form action="index.php?act=goods" method="post">
    ...
    document.write ('<input name="chota" type="hidden" value="<?php echo $attributes[barcode];?>"></form>');
