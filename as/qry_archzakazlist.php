...
$query = "SELECT DISTINCT a.id, DATE_FORMAT( a.time, '%d.%m.%y' ) zakaz_date, a.status, g.price_id, p.comment price_name, u.surname, u.email, a.tags
FROM arch_zakaz AS a, arch_goods AS g, price AS p, customers AS u
WHERE u.id = a.customer
AND a.id = g.zakaz
AND p.id = g.price_id
ORDER BY id DESC";
